...
head:
  - tag: meta
    attrs:
      name: keywords
      content: syncraft production, local-first production checklist, IndexedDB production, offline-first deployment, state sync production
---
...
For React apps, wrap your tree in an [Error Boundary](https://react.dev/reference/react/Component#catching-rendering-errors-with-an-error-boundary) to catch unexpected throws. See the [Error Handling guide](/guides/error-handling) for complete patterns.
...
```tsx
function useAuthSync<T extends Record<string, unknown>>(key: string, opts: UseSyncOptions<T>) {
