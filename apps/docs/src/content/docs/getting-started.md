...
head:
  - tag: meta
    attrs:
      name: keywords
      content: syncraft installation, syncraft quick start, syncraft react, syncraft vue
---
...
export function App() {
  return (
    // Isolate state per request to avoid data leaks in SSR
    <SyncraftProvider>
      <TodoApp />
    </SyncraftProvider>
  );
